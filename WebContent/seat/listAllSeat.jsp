			...
			<div class="container">
                <div class="row">
                	...
                	<table class="table table-striped">
						...
						<a href='addSeat.jsp' class="aLink">新增</a>
						</div>
					
						<c:forEach var="seatVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
							<tr>
								<td>${seatVO.seatCode}</td>
								<td>
									${mapSvc.getSeatTypeByKey(seatVO.seatType).seatTypeName}
								</td>
								
								<td>
									${mapSvc.getSeatStatusByKey(seatVO.seatStatus).seatStatusName}
								</td>
								
								<td>
									<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/seat/SeatServlet" style="margin-bottom: 0px;">
										<input type="submit" value="修改">
										<input type="hidden" name="seatCode"  value="${seatVO.seatCode}">
										<input type="hidden" name="action"	value="getItem_for_update">
									</FORM>
								</td>
								<td>
								  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/seat/SeatServlet" style="margin-bottom: 0px;">
